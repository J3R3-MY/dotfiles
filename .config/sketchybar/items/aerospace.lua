...
local workspaces_names = {"1", "2", "3", "4", "B", "T", "O", "U"}
...
local function updateWindows(workspace_index)
  ...
  sbar.exec(get_windows, function(open_windows)
    ...
    sbar.animate("tanh", 10, function()
      ...
      workspaces[workspace_index]:set({
        icon = { drawing = true },
        label = { drawing = true, string = icon_line, color = colors.blue },
        background = { drawing = true },
        padding_right = 1,
        padding_left = 2,
      })
...
for i, workspace_index in ipairs(workspaces_names) do
  ...
  workspace:subscribe("aerospace_workspace_change", function(env)
      focused_workspace_index = env.FOCUSED_WORKSPACE:gsub("%s+", "")
  ...
  sbar.exec("aerospace list-workspaces --focused", function(focused_workspace)
      focused_workspace = focused_workspace:gsub("%s+", "")
      if workspaces[focused_workspace] then
          ...
          print("WARNING: focused workspace invalid:", focused_workspace)
      end
  end)
end
